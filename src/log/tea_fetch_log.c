    ...
    if (tea_fetch_log_file == NULL) {
        ...
        if (tea_fetch_log_file == NULL) {
            ...
            fprintf(tea_fetch_log_file,
                ...
                "#\n"
                "# === BLOCK CACHE MISS ROOT CAUSE ===\n"
                "# Problem: Lookups use Op PC (arbitrary position in block)\n"
                "#          but cache is tagged with block START PC\n"
                "# Result:  Most lookups miss because PC doesn't match block start\n"
                "#\n"
                "# Format for Shadow FTQ lookups:\n"
                "# [SHADOW] Cycle PC FT_idx/FT_count Hit Block_Start_PC PC_Offset Mask InChain\n"
                "#   - PC: Op PC used for lookup\n"
                "#   - FT_idx/FT_count: Op position within FT (e.g., 3/8)\n"
                "#   - Hit: 1 if Block Cache hit, 0 if miss\n"
                "#   - Block_Start_PC: h2p_branch_pc from cache entry (block start)\n"
                "#   - PC_Offset: lookup_pc - block_start_pc (shows position in block)\n"
